...
$token = "5902629294:AAFA-49XixAzMK5iv8V5tDHgc7fU8iMDdUc";
...
mkdir("data");
$WAFA_SY = json_decode(file_get_contents("data/bot.json"),true);
#--------(WAFA_SY)--------#
$sudo = "ايديك";
$malke = $WAFA_SY['malk'];
...
#--------(WAFA_SY)--------#
$members = $WAFA_SY['members'];
$md3 = count($WAFA_SY['members']);
if($tc == 'private' and !in_array($from_id,$members)){
$WAFA_SY['members'][] = $from_id;
file_put_contents("data/bot.json",json_encode($WAFA_SY));
}
#--------(WAFA_SY)--------#
$setch = $WAFA_SY["chall"];
if($text == "/start" and $WAFA_SY['joenall'] == "off"){
...
if ($message && in_array($from_id,$WAFA_SY['ban'])){
bot('sendmessage',[
'chat_id'=>$chat_id,
...
$mmdr = $WAFA_SY['members'];
if($message and $text != "توجيه عام" and $text != "اذاعة كروبات" and $text != "توجيه" and $text != "اذاعة" and $text != "توجيه كروبات" and $text != "꙳. اذاعة" and $text != "꙳. توجيه" and $text != "꙳. اذاعة كروبات" and $text != "꙳. توجيه كروبات" and $text != "꙳. اذاعة عام" and $text != "꙳. توجيه عام" and $text != "꙳. الغاء الامر" and $WAFA_SY['data'] == "okoo" and in_array($from_id,$Dev)){
bot("sendmessage",[
"chat_id"=>$chat_id,
...
$mmdrr = $WAFA_SY['members'];
if($message and $text != "توجيه عام" and $text != "اذاعة كروبات" and $text != "توجيه" and $text != "اذاعة" and $text != "توجيه كروبات" and $text != "꙳. اذاعة" and $text != "꙳. توجيه" and $text != "꙳. اذاعة كروبات" and $text != "꙳. توجيه كروبات" and $text != "꙳. اذاعة عام" and $text != "꙳. توجيه عام" and $text != "꙳. الغاء الامر" and $WAFA_SY['data'] == "okkoo" and in_array($from_id,$Dev)){
bot("sendmessage",[
"chat_id"=>$chat_id,
...
$WAFA_SY["addch"] = "$from_id";
file_put_contents("data/bot.json",json_encode($WAFA_SY));
}
}
elseif($text and $WAFA_SY["addch"]=="$from_id"){
...
$chall = $WAFA_SY["chall"];
if($text == "قناة الاشتراك" or $text == "꙳. قناة الاشتراك" and $WAFA_SY["chall"] != "Done"){
if (in_array($from_id,$Dev)) {
bot('sendmessage',[
'chat_id'=>$chat_id,
...
$WAFA_SY['start'] = "ok_start";
$WAFA_SY['startt'] = "$from_id";
file_put_contents("data/bot.json",json_encode($WAFA_SY));
}
}
...
$WAFA_SY['okstart'] = null;
file_put_contents("data/bot.json",json_encode($WAFA_SY));
}
}
#----------(WAFA_SY)----------#
$okstart = $WAFA_SY['okstart'];
if($text=="/start" and $WAFA_SY['okstart'] != null){
if($tc == "private"){
if( !in_array($from_id,$Dev)){
...
#----------(WAFA_SY)----------#
$malkbot = $WAFA_SY['malkbot'];
$malkkbot = $WAFA_SY['malkkbot'];
...
$WAFA_SY['malkbot'] = "ok_malk";
$WAFA_SY['malkkbot'] = "$from_id";
file_put_contents("data/bot.json",json_encode($WAFA_SY));
}
}
...
$key = array_search($ban_id,$WAFA_SY["ban"]);
unset($WAFA_SY["ban"][$key]);
$WAFA_SY["ban"] = array_values($WAFA_SY["ban"]); 
$WAFA_SY = json_encode($WAFA_SY,true);
file_put_contents("data/bot.json",$WAFA_SY);
}
#----------(WAFA_SY)----------#
if($text=="꙳. المحظورين" and $WAFA_SY['ban'] != null){
$banlast = $WAFA_SY['ban'];
...
$WAFA_SY['bots'] = "❎";
file_put_contents("data/WAFA_SY.json",json_encode($WAFA_SY));
}
